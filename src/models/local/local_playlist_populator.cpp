...
void LocalPlaylistPopulator::run()
{
    ...
    /* ----------------------------------------------------------*/
    QSqlQuery queryCount("SELECT COUNT(*) FROM `playlist_items`",*db.sqlDb());
    queryCount.next();
    ...
    QSqlQuery query("SELECT playlist_id,playlist_filename,playlist_name,playlist_type,playlist_favorite,playlist_mtime, \
                            url,name,track_id \
                     FROM view_playlists \
                     ORDER BY playlist_type ASC",*db.sqlDb());
    ...
    /* ----------------------------------------------------------*/
    QSqlQuery query_2("SELECT id,name,icon,rules,type,favorite FROM smart_playlists ORDER BY type ASC",*db.sqlDb());
    ...
    while (query_2.next() && !m_exit) {
      ...
      playlistItem->rules      =  query_2.value(3);
