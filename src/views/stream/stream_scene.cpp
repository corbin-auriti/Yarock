...
void StreamScene::initScene()
{
    ...
    layout->addWidget(ui_line_edit);

    ui_proxy = this->addWidget(ui_search_widget, Qt::Widget);
    ui_proxy->setSizePolicy( QSizePolicy::Expanding, QSizePolicy::Expanding );
...
/*******************************************************************************
     setFilter
*******************************************************************************/
void StreamScene::setFilter(const QString& filter)
{
...
void StreamScene::populateExtendedStreamScene()
{
    ...
    /*  header item */
    HeaderItem* header = new HeaderItem(28, qobject_cast<QGraphicsView*> (parentView())->viewport());
    header->setText( m_services[mode()]->name() );
    header->setPos(0, Ypos);
...
void StreamScene::playStream()
{
    ...
    /* urls simple                                      */
    /* -------------------------------------------------*/
    if( !MEDIA::isPlaylistFile(streamUrl) &&
        !MEDIA::isShoutCastUrl(streamUrl) &&
        !MEDIA::isTuneInUrl(streamUrl) )
    {
...
void StreamScene::slot_contextmenu_triggered(ENUM_ACTION_ITEM_MENU id)
{
    switch(id)
    {
      ...
      case STREAM_QUEUE_NEW    : enqueueStream(true);     break;
