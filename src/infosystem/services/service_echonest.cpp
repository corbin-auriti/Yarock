...
void ServiceEchonest::fetch_artist_bio(INFO::InfoRequestData request)
{
    ...
    if ( hash.contains( "artist" ) )
    {
        ...
        url.addQueryItem("api_key", Echonest::API_KEY);
        url.addEncodedQueryItem("name",  Echonest::escapeSpacesAndPluses(hash["artist"]));
        url.addEncodedQueryItem("results", QByteArray::number(1) );
...
void ServiceEchonest::slot_get_artist_biography(QByteArray bytes)
{
    ...
    if (resp_map.contains("status") && resp_map.contains("biographies"))
    {
        ...
        foreach (const QVariant& bio, resp_map.value("biographies").toList()) {
          if(bio.toMap().value("site").toString() == "lastfm") {
            foundBio = bio.toMap();
            break;
          }
        }
...
void ServiceEchonest::fetch_artist_similar(INFO::InfoRequestData request)
{
    ...
    url.addQueryItem("api_key", Echonest::API_KEY);
    url.addEncodedQueryItem("name",  Echonest::escapeSpacesAndPluses(hash["artist"]));
    url.addQueryItem("bucket", "images");
    url.addQueryItem("min_hotttnesss", "0.5");
    url.addEncodedQueryItem("results", QByteArray::number(6) );
    url.addQueryItem("format", "json");
