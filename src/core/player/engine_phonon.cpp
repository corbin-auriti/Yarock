...
void EnginePhonon::setMediaItem(MEDIA::TrackPtr track)
{
    ...
    if(Phonon::MediaSource(path).type() != Phonon::MediaSource::Invalid)
      m_mediaObject->setCurrentSource( Phonon::MediaSource(path) );
    else
      m_currentMediaItem->isBroken = true;
...
void EnginePhonon::setNextMediaItem(MEDIA::TrackPtr track)
{
    ...
    if( MEDIA::isLocal(path) )
      m_mediaObject->enqueue( Phonon::MediaSource(path) );
    else
      m_mediaObject->enqueue( Phonon::MediaSource(QUrl(path)) );
...
void  EnginePhonon::slot_on_media_finished()
{
    ...
    else if( m_mediaObject->queue().isEmpty())
    {
      ...
      // emit engineQueueFinished();
    }

    /*
    //! DEBUG check queue
    Debug::debug() << "[PLAYER] m_mediaObject->queue().size() :" << m_mediaObject->queue().size() ;
    foreach (Phonon::MediaSource source, m_mediaObject->queue() )
    {
     Debug::debug() << "[PLAYER] #queue -> url :" << source.url() ;
    }
    */
}
