...
static void maybeAddPeak( const TagLib::String &scaleVal, MEDIA::ReplayGainTag key, MEDIA::ReplayGainTagMap *map )
{
...
static void maybeAddGain( const TagLib::String &input, MEDIA::ReplayGainTag key, MEDIA::ReplayGainTagMap *map )
{
...
static MEDIA::ReplayGainTagMap readID3v2Tags( TagLib::ID3v2::Tag *tag )
{
    MEDIA::ReplayGainTagMap map;
            ...
            for ( unsigned int i = 0; i < frames.size(); ++i )
            {
                ...
                if ( channel == 1 )
                {
                    ...
                    if ( ok )
                    {
                        ...
                        {
                            map[MEDIA::ReplayGain_Album_Gain] = adjustment;
                            map[MEDIA::ReplayGain_Album_Peak] = peakToDecibels( peak );
                        }
                        else if ( desc.toLower() == "track" || !map.contains( MEDIA::ReplayGain_Track_Gain ) )
                        {
                            map[MEDIA::ReplayGain_Track_Gain] = adjustment;
                            map[MEDIA::ReplayGain_Track_Peak] = peakToDecibels( peak );
                        }
                    }
                }
            }
        ...
        for ( TagLib::ID3v2::FrameList::Iterator it = frames.begin(); it != frames.end(); ++it ) {
            ...
            if ( frame && frame->fieldList().size() >= 2 )
            {
                ...
                if ( desc == "replaygain_album_gain" )
                    maybeAddGain( frame->fieldList()[1], MEDIA::ReplayGain_Album_Gain, &map );
                if ( desc == "replaygain_album_peak" )
                    maybeAddPeak( frame->fieldList()[1], MEDIA::ReplayGain_Album_Peak, &map );
                if ( desc == "replaygain_track_gain" )
                    maybeAddGain( frame->fieldList()[1], MEDIA::ReplayGain_Track_Gain, &map );
                if ( desc == "replaygain_track_peak" )
                    maybeAddPeak( frame->fieldList()[1], MEDIA::ReplayGain_Track_Peak, &map );
            }
        }
    }
    return map;
...
static MEDIA::ReplayGainTagMap readAPETags( TagLib::APE::Tag *tag )
{
    MEDIA::ReplayGainTagMap map;
    const TagLib::APE::ItemListMap &items = tag->itemListMap();
    if ( items.contains("REPLAYGAIN_TRACK_GAIN") )
    {
        maybeAddGain( items["REPLAYGAIN_TRACK_GAIN"].values()[0], MEDIA::ReplayGain_Track_Gain, &map );
        if ( items.contains("REPLAYGAIN_TRACK_PEAK") )
            maybeAddPeak( items["REPLAYGAIN_TRACK_PEAK"].values()[0], MEDIA::ReplayGain_Track_Peak, &map );
    }
    if ( items.contains("REPLAYGAIN_ALBUM_GAIN") )
    {
        maybeAddGain( items["REPLAYGAIN_ALBUM_GAIN"].values()[0], MEDIA::ReplayGain_Album_Gain, &map );
        if ( items.contains("REPLAYGAIN_ALBUM_PEAK") )
            maybeAddPeak( items["REPLAYGAIN_ALBUM_PEAK"].values()[0], MEDIA::ReplayGain_Album_Peak, &map );
    }
    return map;
}

static MEDIA::ReplayGainTagMap readXiphTags( TagLib::Ogg::XiphComment *tag )
{
    const TagLib::Ogg::FieldListMap &tagMap = tag->fieldListMap();
    MEDIA::ReplayGainTagMap outputMap;

    if ( !tagMap["REPLAYGAIN_TRACK_GAIN"].isEmpty() )
    {
        maybeAddGain( tagMap["REPLAYGAIN_TRACK_GAIN"].front(), MEDIA::ReplayGain_Track_Gain, &outputMap );
        if ( !tagMap["REPLAYGAIN_TRACK_PEAK"].isEmpty() )
            maybeAddPeak( tagMap["REPLAYGAIN_TRACK_PEAK"].front(), MEDIA::ReplayGain_Track_Peak, &outputMap );
    }

    if ( !tagMap["REPLAYGAIN_ALBUM_GAIN"].isEmpty() )
    {
        maybeAddGain( tagMap["REPLAYGAIN_ALBUM_GAIN"].front(), MEDIA::ReplayGain_Album_Gain, &outputMap );
        if ( !tagMap["REPLAYGAIN_ALBUM_PEAK"].isEmpty() )
            maybeAddPeak( tagMap["REPLAYGAIN_ALBUM_PEAK"].front(), MEDIA::ReplayGain_Album_Peak, &outputMap );
    }

    return outputMap;
}

static MEDIA::ReplayGainTagMap readASFTags( TagLib::ASF::Tag *tag )
{
    ...
    if ( !tagMap["REPLAYGAIN_TRACK_GAIN"].isEmpty() )
    {
        maybeAddGain( tagMap["REPLAYGAIN_TRACK_GAIN"].front().toString(), MEDIA::ReplayGain_Track_Gain, &outputMap );
        if ( !tagMap["REPLAYGAIN_TRACK_PEAK"].isEmpty() )
            maybeAddPeak( tagMap["REPLAYGAIN_TRACK_PEAK"].front().toString(), MEDIA::ReplayGain_Track_Peak, &outputMap );
    }

    if ( !tagMap["REPLAYGAIN_ALBUM_GAIN"].isEmpty() )
    {
        maybeAddGain( tagMap["REPLAYGAIN_ALBUM_GAIN"].front().toString(), MEDIA::ReplayGain_Album_Gain, &outputMap );
        if ( !tagMap["REPLAYGAIN_ALBUM_PEAK"].isEmpty() )
            maybeAddPeak( tagMap["REPLAYGAIN_ALBUM_PEAK"].front().toString(), MEDIA::ReplayGain_Album_Peak, &outputMap );
    }
...
static MEDIA::ReplayGainTagMap readMP4Tags( TagLib::MP4::Tag *tag )
{
    MEDIA::ReplayGainTagMap outputMap;

    if ( !tag->trackReplayGain().isNull() ) {
        maybeAddGain( tag->trackReplayGain(), MEDIA::ReplayGain_Track_Gain, &outputMap );
        if ( !tag->trackReplayGainPeak().isNull() )
            maybeAddPeak( tag->trackReplayGainPeak(), MEDIA::ReplayGain_Track_Peak, &outputMap );
    }

    if ( !tag->albumReplayGain().isNull() ) {
        maybeAddGain( tag->albumReplayGain(), MEDIA::ReplayGain_Album_Gain, &outputMap );
        if ( !tag->albumReplayGainPeak().isNull() )
            maybeAddPeak( tag->albumReplayGainPeak(), MEDIA::ReplayGain_Album_Peak, &outputMap );
    }
...
MEDIA::ReplayGainTagMap
MEDIA::readReplayGainTags( TagLib::FileRef fileref )
{
    MEDIA::ReplayGainTagMap map;
